$(window).on("load", function () {
  ...
  $.get("csv/Options.csv", function (options) {
    ...
    if (typeof googleDocURL !== "undefined" && googleDocURL) {
      if (typeof googleApiKey !== "undefined" && googleApiKey) {
        ...
        $.when(
          $.ajax({
            url: apiUrl + spreadsheetId + "/values/Options?key=" + googleApiKey,
            method: "GET",
            dataType: "json",
          }),
          $.ajax({
  ...
  function parseGeoJsonUrls(urlValue) {
    if (!urlValue) {
      return [];
    }
      ...
      for (var i = 0; i < pixelsAbove.length - 1; i++) {
        if (
          ...
          if (c["Overlay"]) {
            ...
            var geoJsonOverlayUrls = parseGeoJsonUrls(url);
            var hasOnlyGeoJsonOverlays =
              geoJsonOverlayUrls.length > 0 &&
              geoJsonOverlayUrls.every(function (overlayUrl) {
                return isGeoJsonUrl(overlayUrl);
              });

            if (hasOnlyGeoJsonOverlays) {
              overlay = L.featureGroup().addTo(map);

              geoJsonOverlayUrls.forEach(function (overlayUrl) {
                $.getJSON(overlayUrl, function (geojson) {
                  L.geoJson(geojson, {
          ...
          if (c["GeoJSON Overlay"]) {
            var geoJsonUrls = parseGeoJsonUrls(c["GeoJSON Overlay"]);
            ...
            if (geoJsonUrls.length > 0) {
              ...
              geoJsonOverlay = L.featureGroup().addTo(map);
                  ...
                  L.geoJson(geojson, {
                    style: function (feature) {
                      return {
                        ...
                        opacity:
                          feature.properties.opacity || props.opacity || 0.5,
                        color:
                          feature.properties.color || props.color || "#cccccc",
                        fillOpacity:
